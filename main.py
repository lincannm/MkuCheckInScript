...
def main():
    ...
    # -p/--prompt：选中账号后先整批询问确认，不直接打卡
    if args.prompt and not args.only_screenshot:
        today_str = date.today().strftime('%Y-%m-%d')
        if not get_choose(f"今天日期：{today_str}\n确认对以上 {len(selected_accounts)} 个账号执行打卡？"):
            print("用户取消打卡，结束程序")
            sys.exit(0)
